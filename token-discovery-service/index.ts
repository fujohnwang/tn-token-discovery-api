...
export class TokenDiscoveryService {
  ...
  private getOnChainAPISupportBool(apiName: string, chain: string) {
  ...
  */
  public async getTokens(
    ...
    if (openSeaSlug !== undefined) {
      let openseaTokens = await this.getTokensOpenSea(
        smartContract,
        chain,
        owner,
        openSeaSlug
    ...
    if (!this.validateTokenMetadata(tokens)) {
      let moralisTokens = await this.getTokensMoralis(
        smartContract,
        chain,
        owner
      );
    ...
    if (!this.validateTokenMetadata(tokens) && openSeaSlug === undefined) {
      let alchemyTokens = await this.getTokensAlchemy(
        smartContract,
        chain,
        owner
      );
  ...
  private validateTokenMetadata(tokens: {
    [tokenId: string]: IToken;
  }): boolean {
  ...
  private async httpJsonRequest(req: string, requestOptions: any) {
